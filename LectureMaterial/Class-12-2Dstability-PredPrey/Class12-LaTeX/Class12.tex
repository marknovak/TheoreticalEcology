...
\begin{center}
	\begin{tabular}{ll}
		...
		$Re(\lambda_i)< 0$ for some $i$ & Saddle point \\ 
...
\begin{equation*}
	f(R)=\frac{\hat{a}R^m}{1+\hat{a}hR^m}
\end{equation*}
...
\ind e.g., Let $a = \hat{a}R$ (preference increases with density), then get 
$f(R)=\frac{\hat{a}R^2}{1+\hat{a}hR^2}$.\\
Note that units of $\hat{a}$ are different than those of $a$!\\
...
\textbf{Rosenzweig-MacArthur Model (1963)} (a.k.a. Paradox of enrichment model)
...
For Rosenzweig-MacArthur model:
\begin{equation*}
...
At given parameter values (other responses possible)
\begin{equation*}
\begin{rcases}
  \text{decreasing }d & \\
  \text{increasing }e & \\
