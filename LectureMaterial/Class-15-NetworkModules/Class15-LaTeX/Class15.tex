...
\begin{enumerate}
	...
	\item What level of productivity ($K$) is needed for each consumer to invade (\emph{symbolically})?
\end{enumerate}
...
\begin{align*}
	\frac{1}{R}\frac{dR}{dt}=r-\frac{rR}{K}-a_{RC}C = 0 \qquad \Rightarrow \qquad C_R^* = \frac{r-\frac{r}{K}R}{a_{RC}} = \frac{r}{a_{RC}}-\frac{r}{K a_{RC}}R
\end{align*}
\ind Conclusion:\\
\ind \ind $C$ can invade when $K > R_C^*$.\\
...
\ind RC equilibria:\\
\begin{align*}
	R_{eq}^*&=\frac{m_C}{e_{RC}a_{RC}}\\
	\\
	C_{eq}^*&= \frac{r-\frac{r}{K}R^*}{a_{RC}} = \frac{r-\frac{r}{K}\frac{m_C}{a_{RC}e_{RC}}}{a_{RC}} = \frac{r}{a_{RC}}-\frac{r m_C}{a_{RC}^2 e_{RC} K} = \frac{r(a_{RC}e_{RC}K-m_C)}{a_{RC}^2 e_{RC} K}
\end{align*}
...
\begin{align*}
	...
	\frac{m_O a_{RC}}{e_{CO} a_{CO}}  & = r - \frac{ r m_C}{a_{RC}e_{RC} K}\\
	\Rightarrow & \quad   r - \frac{ r m_C}{a_{RC}e_{RC} K} - \frac{m_O a_{RC}}{e_{CO} a_{CO}}  \geq 0
\end{align*}
\begin{equation*}
