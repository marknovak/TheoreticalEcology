...
\begin{center}
\includegraphics[width=12.5cm]{figs/EqnConnections.pdf}
\end{center}
\rule[0.5ex]{\linewidth}{1pt}
...
\begin{itemize}
	\item[] $b_d$ - births per individual
	\item[] $d_d$ - deaths per individual per time step (i.e. probability of each individual dying)
\end{itemize}
...
\begin{itemize}
	\item[] $r_d$ - discrete growth factor/increment (Ted Case writes this as $R$).
	\item[] $\lambda$ - “finite rate of increase” - per capita rate of growth if population is growing 
	geometrically.
\end{itemize}

\vspace{0.8cm}

\note{Draw $N(t)$ vs. $t$ on arithmetic scale on board in steps} \note{(4 y-axis increments)}
\begin{center}
...
\note{A:} Logarithms convert multiplicative processes to additive processes.\\
On log-scale, products become sums, ratios become differences:
\begin{center}
  \begin{tabular}{c|c}
...
\note{A:}	List includes:
\begin{itemize}
	\item	synchronous discrete reproduction and mortality
	\item	constant growth rate (deterministic, i.e., non-stochastic)
...
\begin{align*}
	...
	& \lambda = \lim_{n \to \infty}\left(1+\tfrac{r_d}{n}\right)^n = e^r \quad \quad r \text{ - 
...
\begin{align*}
	...
	n \to\infty \Rightarrow & \frac{N_1}{1} = \left(1+\tfrac{1}{n}\right)^n =  2.71828...
\end{align*}
Per capita growth in continuous time is not $r_d$ but rather $e^r$ (with $r < r_d$).

\rule[0.5ex]{\linewidth}{1pt}
\begin{align*}
		\lim_{n \to \infty}\left(1+\tfrac{r_d}{n}\right)^n  &= e^r
\end{align*}

Definition of natural logarithm as the anti-exponential and $e$ as the anti-log: $\ln(e^x)=x$.

Note that $\log=\log_e=\ln$.

The same definition holds  for logarithms in other bases:
\begin{center}
$\log_{10}(10^x)=x \quad \quad \quad   \log_2(2^x)=x$
\end{center}
\rule[0.5ex]{\linewidth}{1pt}
Summarize $r$ vs. $r_d$ vs. $\lambda$
\begin{equation*}
	(1+\frac{r_d}{1})=\lambda=e^r
\end{equation*}
...
\begin{center}
\includegraphics[width=8cm]{figs/image2.png}
\end{center}
...
\begin{equation*}
\lim_{\Delta t \to 0}\left(\frac{\Delta N_t}{\Delta t}\right) = \frac{dN_t}{dt}
\end{equation*}
...
\begin{align*}
	...
	&=   r \cdot N_0 e^{rt} \\
\text{Since } N_t=N_0 e^{rt} \text{ for any time } t ...&\\
	&=rN_t=\frac{dN_t}{dt} \quad \text{...then drop $t$ subscript}
\end{align*}
...
&	\int_0^T \frac{1}{N}\frac{dN}{dt} \; dt = \int_0^T r\; dt \;\;\;\;(\text{Think of }T \text{ as a constant, 
and }t\text{ in }dt\text{ as a variable})\\
&	\int_0^T \frac{1}{N}\frac{dN}{dt} \; dt = r t \vert_0^T = r \cdot T - r \cdot 0 = r T\\
\end{align*}
Using $\int \frac{1}{x}\; dx = \ln(x)$.  That is,  $\int_{x_0}^{x_T} \frac{1}{x}\; dx = \ln(x_T)-\ln(x_0)$, we 
have that
\begin{align*}
&	ln(N_T)-ln(N_0)=rT\\
