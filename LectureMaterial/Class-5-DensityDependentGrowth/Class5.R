...
#########################
# N(t) vs. t
quartz(width = 14 * qm, height = 3.5 * qm)
par(mfrow = c(1, 4))
par(
  mar = c(4, 4, 1.5, 1),
  lwd = 2,
...
############################
# N(t+1) vs N(t)
quartz(width = 12 * qm, height = 4 * qm)
par(mfrow = c(1, 3))
par(
  mar = c(4, 4, 1.5, 1),
  lwd = 2,
...
plot(
  y ~ N[-length(N)],
  ylab = expression(Delta * N[Delta * t]),
  xlab = expression(N[t]),
  type = 'l',
  xlim = c(0, 110),
  lwd = 4,
