...
# Define a function for the determenistic discrete logistic whose input parameters
...
out<-DlogisK(N0=0.01,T=30,rd=0.5,K=100)
quartz()
...
out2<-data.frame(ode(y=c(N=1),times=t,func=ClogisK,parms=c(r=0.2, K=100)))
N2<- out2$N; dNdt2<- N2[-1]-N2[-length(N2)]

# Different carrying capacity
out3<-data.frame(ode(y=c(N=1),times=t,func=ClogisK,parms=c(r=0.2, K=110)))
...
quartz(height=4,width=7)
par(mfrow=c(1,2))
	plot(N~time,data=out,type='l',lwd=4,ylim=c(0,150))
	points(N~time,data=out2,type='l',lwd=4,col='red')
	points(N~time,data=out3,type='l',lwd=4,col='blue')

	plot(dNdt~N[-length(N)],type='l',lwd=4,xlab='N',ylim=c(-2,6),xlim=c(0,150))
	points(dNdt2~N2[-length(N2)],type='l',lwd=4,col='red')
	points(dNdt3~N3[-length(N3)],type='l',lwd=4,col='blue')
	abline(0,0,col='grey')
