############################################################
############################################################
# Contrasting the effects of assuming observation vs. process error and model-fitting and calculation of AIC
############################################################
...
dat<-read.csv('Class6-data/safseal1.csv')
# or use: dat <- read.csv(file.choose())
...
# lambda = e^r, thus corresponds to the exponentiated 'b' coefficient of 'y=a+b*x'
r_obs <- coef(fit)[2]
...
plot(Nobs[-T], log_lambda_obs, 
     ylab = 'log(Nt+1/Nt)', xlab = 'N')
...
Np[, 1] <-
  Nobs[T] # Starting population size is last observed population size
No <- Np
...
plot(
  Nobs ~ Year,
  xlim = c(min(Year), max(Year) + FutureTime),
  ylim = c(min(Nobs), max(Np)),
  type = 'l',
  log = 'y',
  axes = FALSE
)
