...
quartz(width = 4 * qm, height = 4 * qm)
plot(lambda,
     Prob,
     ylab = 'Probability',
     xlab = 'Possible lambdas',
     cex = 0.5)
...
###########################################
# NOTE:
# We're going to use nls() rather than lm() so that you can accommodate the (nonlinear) theta-logistic model of Problem set #3
############################################################
############################################################
rm(list = ls()) # clears workspace
qm <- 2
...
# (using nonlinear least squares for consistency with next)
DIfit <- nls(y ~ b1 + 0 * x,
...
################################################
# Repeat by optimizing own log-Likelihood function
#################################################
...
# optimize parameters p to minimize nLL function
fit <- optim(p, nLL)
