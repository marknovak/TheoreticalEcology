...
# Examine the effects of minutely different N0 on the population size at last 50 time points (1950 through 2000)
...
for (rd in rd.vals) {
  out <- Dlogis(rd = rd,
                ...
                N0 = 0.01,
                T = 2000)
# use only the last 10% of time-steps to avoid transients
  l <- length(out) %/% 10 
  out <- out[(9 * l):(10 * l)]
  p <- peaks(out)
...
for (r in seq(1, 3, 0.001)) {
  ...
  l <-
    length(out) %/% 10 # use only the last 10% of time-steps to avoid transients
  out <- out[(9 * l):(10 * l)]
  p <- peaks(out)
