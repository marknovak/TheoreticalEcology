...
abline(0, 1, lwd = 3)
text(13, 13, 'f(x)=x', srt = 45, pos = 3)
grid(15, 15, col = 'black')
legend('top',
       horiz = T,
       legend = bquote(list(K == .(K), r[d] == .(rd))),
       bg = 'white')
par(op)
...
for (i in 1:2) {
  for (rd in c(1.5, 2.3, 2.5)) {
    curve(f, 0, 18, ylim = c(0, 16), lwd = 5,
          xlab = expression(N[t]),
          ylab = expression(F(N[t])))
    abline(0, 1, lwd = 3)
    text(13, 13, expression(F(N[t])==N[t]), srt = 45, pos = 3)
    grid(15, 15, col = 'black')
    legend(
      'top',
      horiz = T,
