...
\includegraphics[width=4cm]{figs/RickerSlope.jpg}
\end{center}
...
\note{Class Q:} What about $N^* = 0$?  (Stick in to get $F'(0) = r$ $\implies$ unstable).
...
\begin{align*}
	n_{t+1}& =F(N^*+n_t)-N^*\\
	& \approx \left [ F(N^*)+ F'(N^*) \cdot n_t \right ] - N^*\\
	& \approx N^* + F'(N^*) \cdot n_t - N^*\\
	& \approx F'(N^*) \cdot n_t \\
	& = \lambda n_t \quad \quad (\lambda \text{ is the ``eigenvalue'')}
\end{align*}
...
\ind \ind $N : \#$
\ind \ind $K : \#$
\ind (Remember $K=\frac{r}{\alpha}$ and $\alpha : \frac{\#}{\# \# t}$)
