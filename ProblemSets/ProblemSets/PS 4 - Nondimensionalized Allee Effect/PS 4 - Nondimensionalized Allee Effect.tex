...
\usepackage{graphicx}
\author{Mark Novak}
\title{BI592 Theoretical Ecology \\ PS 4. Logistic with Allee effect}
\date{}
\author{}
\begin{document}
\maketitle

\section{Non-dimensional analysis of logistic growth with Allee effect}
...
We can non-dimensionalize this Allee model to reduce its size by scaling the state variable and grouping the parameters as follows: 
\begin{equation}
x=\frac{N}{K},\;\;\tau=rt\;\;\beta=\frac{A}{K}.
\end{equation}
$x$ is now a dimensionless variable (i.e., has no units) and represents the proportional population size (proportional to the population's carrying capacity). $\beta$ is a dimensionless parameter and represents the minimum proportion that is needed for positive growth. $\tau$ is a dimensionless reflection of time.
