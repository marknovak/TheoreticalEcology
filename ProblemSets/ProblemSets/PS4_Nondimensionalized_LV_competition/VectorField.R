...
x.exp <- xy.pairs$x
y.exp <- xy.pairs$y
list(values=matrix(vecfun(x.exp,y.exp), nrow=2, byrow=TRUE), x=x.exp, y=y.exp)
}

# vector field plot function
# grid.points can be defined for both axes at once or separately
plotVectorField <- function(vecfun, xlim, ylim, arrow.length=0.05, grid.points=c(10,10)) {
gp <- if(length(grid.points)>1) grid.points
else rep(grid.points,2)
...
plot.default(range(x0,x1), range(y0,y1), xlab="", ylab="", type="n", frame.plot=T)
arrows(x0,y0,x1,y1,length = arrow.length, angle = 20, code = 2)
}
