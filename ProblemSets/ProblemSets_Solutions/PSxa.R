...
sd_o <- 0.1 # observation error (standard dev. of estimated log popn sizes)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# NOTE:  In the following we're going to use a single for-loop to run both of our simulations simultaneously!
# That is, we'll save one set of results assuming only observation error in "N_oe", and another set of
# (completely independent) results assuming only process error in "N_pe".  
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Initialize variables to store population counts and growth rates
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Time<-array(0,dim=c(reps,T)) # empty array of time-points
Time[,1]<-1 # starting time point
N<-array(0,dim=c(reps,T)) # empty array of population sizes
N_pe<-N  # empty array of population sizes for process error only results
N_oe<-N # empty array of population sizes for observation error only results
N[,1]<-N0;  # initial set of population sizes across replicate populations
N_pe[,1]<-N0 # initial set of population sizes for process error only
N_oe[,1]<-exp(rnorm(reps,log(N[,1]),sd_o)) # initial set of population sizes with obervation error
grate_pe<-array(0,dim=c(reps,(T-1))) # container for growth rates assumine process error only
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Define some functions for use in plotting and fitting
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
print(paste('With process error, estimated lambda_g = ',round(lam_est_pe,5),' estimated sigma_p = ', round(sd_pe_est,5)))
...
print(paste('With observation error, estimated lambda = ',round(lam_est_oe,5),' estimated sigma_o = ',round(sd_oe_est,5),' and estimated N0 = ',round(N0_est,5)))
